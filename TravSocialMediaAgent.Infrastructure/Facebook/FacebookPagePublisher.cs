...
internal sealed class FacebookPagePublisher(
    ...
    ILogger<FacebookPagePublisher> logger) : ISocialPostPublisher
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    public async Task<string> PublishAsync(TravSocialPost post, CancellationToken cancellationToken)
    {
        var pageId = ConfigurationValue.Require(facebookOptions.PageId, "Facebook:PageId or FACEBOOK_PAGE_ID");
        ...
        using var response = await httpClient.PostAsync($"{Uri.EscapeDataString(pageId)}/feed", content, cancellationToken);
        ...
        logger.LogWarning("Facebook Graph API accepted the post but did not return a post id. Status: {StatusCode}", response.StatusCode);
        return response.StatusCode == HttpStatusCode.OK ? "unknown" : response.StatusCode.ToString();
    }
